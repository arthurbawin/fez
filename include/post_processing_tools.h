...
namespace PostProcessingTools
{
  ...
  public:
     ...
     * Constructor.
     */
    PostprocessorAtDofBase(const ComponentOrdering    &ordering,
     ...
     * there is a do_postprocess
     * function with the required concrete types for @p present_solution. The derived classes
     * can implement a templatized do_postprocess, and simply forward the calls
     * of the do_postprocess'es with concrete types to this template.
     */
    template <typename VectorType>
    void postprocess(const VectorType &present_solution)
    {
      do_postprocess(present_solution);
    }

    /**
     * Add the data stored in "solution" to the DataOut of the passed @p postproc_handler.
     */
  ...
  protected:
     ...
     */
    virtual void do_postprocess(const LA::ParVectorType &present_solution) = 0;
