...
namespace Assembly
{
  namespace IncompressibleCHNS
  {
                      ...
                      CopyData          &copy_data) const
    {
      ...
      constexpr bool with_ale_stabilization =
        BaseType::with_moving_mesh &&
        (BaseType::with_stabilization || BaseType::with_tracer_stabilization);
      std::vector<Tensor<1, dim>> strong_residual_momentum_x_variation(
        BaseType::with_moving_mesh && BaseType::with_stabilization ?
          sd.dofs_per_cell :
          0);
      std::vector<double> strong_residual_tracer_x_variation(
        BaseType::with_moving_mesh && BaseType::with_tracer_stabilization ?
          sd.dofs_per_cell :
          0);
      std::vector<Tensor<1, dim>> convective_velocity_x_variation(
        with_ale_stabilization ? sd.dofs_per_cell : 0);
      std::vector<double> trace_grad_phi_x_moving(
        with_ale_stabilization ? sd.dofs_per_cell : 0);
