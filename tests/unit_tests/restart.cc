...
void test_restart()
{
  ...
  // Run the full computation from t = 0 to 0.5, checkpointing once at t = 0.3
  {
    ...
    deallog << "Error on velocity without restart" << std::endl;
    for (const auto &[t, e] : e_u)
      deallog << t << " : " << e << std::endl;
    deallog << "Error on pressure without restart" << std::endl;
    for (const auto &[t, e] : e_p)
      deallog << t << " : " << e << std::endl;
  }
  ...
  const unsigned int time_steps_shift = 3;
  {
    ...
    deallog << "Error on velocity with restart" << std::endl;
    for (const auto &[t, e] : e_u_restart)
      deallog << t << " : " << e << std::endl;
    deallog << "Error on pressure with restart" << std::endl;
    for (const auto &[t, e] : e_p_restart)
      deallog << t << " : " << e << std::endl;
  }
