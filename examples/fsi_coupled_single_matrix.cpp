...
namespace fsi_coupled
{
  ...
  /**
   * These data should be handled by the parameter reader,
   * which requires implementing support for boundary conditions.
  ...
  public:
                              ...
                              Vector<double>   &values) const override
    {
      ...
      for(unsigned int d = 0; d < dim; ++d)
      {
        values[x_lower + d] = p[d];
        if(d == 1)
          values[x_lower + d] = p[d] + H / 50. * sin(0.1 * M_PI * t);
      }
    }
  };
  ...
  template <int dim>
  FSI<dim>::FSI(ParameterReader<dim> &param,
                BoundaryDescription  &boundary_description)
    : param(param)
    , boundary_description(boundary_description) // TO REMOVE
    ...
    , fe(FE_SimplexP<dim>(param.finite_elements.velocity_degree), // Velocity
         ...
         dim,
         FE_SimplexP<dim>(param.finite_elements.no_slip_lagrange_mult_degree), // Lagrange multiplier
         dim)
    ...
    , current_time(param.time_integration.t_initial)
    ...
    , mesh_position_circle_fun(
        MeshPositionCircle<dim>(current_time, n_components))
  {
  }

  template <int dim>
  void FSI<dim>::set_bdf_coefficients()
  {
    const double dt      = param.time_integration.dt;
    const double prev_dt = dt; // To fix in due time

    switch (param.time_integration.scheme)
    {
      case Parameters::TimeIntegration::Scheme::stationary:
        bdfCoeffs.resize(1);
        bdfCoeffs[0] = 0.;
        break;
      case Parameters::TimeIntegration::Scheme::BDF1:
        bdfCoeffs.resize(2);
        bdfCoeffs[0] = 1. / dt;
        bdfCoeffs[1] = -1. / dt;
        break;
      case Parameters::TimeIntegration::Scheme::BDF2:
        bdfCoeffs.resize(3);
        // bdfCoeffs[0] =  3. / (2. * dt);
        // bdfCoeffs[1] = -2. / dt;
        // bdfCoeffs[2] =  1. / (2. * dt);
  ...
  void FSI<dim>::make_grid()
  {
    ...
    AssertThrow(input, ExcMessage("Could not open mesh file: " + param.mesh.filename));
  ...
  void FSI<dim>::create_lambda_zero_constraints(const unsigned int boundary_id)
  {
    ...
    if (boundary_description.weak_velocity_boundary_names.size() == 0)
        ...
        for (unsigned int q = 0; q < face_quadrature.size(); ++q)
        {
          ...
          for (unsigned int i_dof = 0; i_dof < n_dofs_per_face; ++i_dof)
          {
            ...
            if (is_lambda(comp))
            {
              ...
              coeffs[d][lambda_dof]   += - phi_i * JxW / param.fsi.spring_constant;
            }
    ...
    bool                                                  distribute)
  {
    ...
    const double kinematic_viscosity = param.physical_properties.fluids[0].kinematic_viscosity;
    const double lame_lambda = param.physical_properties.pseudosolids[0].lame_lambda;
    const double lame_mu     = param.physical_properties.pseudosolids[0].lame_mu;
      ...
      for (unsigned int i = 0; i < scratchData.dofs_per_cell; ++i)
      {
        ...
        double local_rhs_ps_i = 
            - ( // Linear elasticity
              ...
              lame_mu * scalar_product(present_displacement_gradient_sym, grad_phi_x[i])
  ...
  template <int dim>
  void FSI<dim>::solve_newton(const bool is_initial_step)
  {
    double global_res;
    double current_res;
    double last_res;
    ...
    while ((global_res > this->param.nonlinear_solver.tolerance) &&
           outer_iteration < 50)
      {
        ...
        if(param.nonlinear_solver.enable_line_search)
        {
          ...
          for (double alpha = 1.0; alpha > 1e-1; alpha *= 0.5)
          {
            ...
            if (current_res < 0.1 * last_res ||
                last_res < param.nonlinear_solver.tolerance)
              {
                break;
              }
    ...
    if ((global_res > param.nonlinear_solver.tolerance) &&
        outer_iteration >= param.nonlinear_solver.max_iterations)
      {
        throw(std::runtime_error(
  ...
  FSI<dim>::create_mask_arrays()
  {
    ...
    std::ofstream outfile(param.output.output_dir + "masks.txt");
                                ...
                                const unsigned int newton_step)
  {
    ...
    else
    {
      ...
      data_out.write_vtu_with_pvtu_record(
        param.output.output_dir, param.output.output_prefix, time_step, mpi_communicator, 2);
    }
  }
  ...
  FSI<dim>::compare_lambda_position_on_boundary(const unsigned int boundary_id)
  {
    ...
    pcout << "                         Ratio is " << ratio << " (expected: " << -param.fsi.spring_constant << ")" << std::endl;
    ...
    for (unsigned int d = 0; d < dim; ++d)
    {
      ...
      const double absolute_error = std::abs(ratio[d] - (-param.fsi.spring_constant));
  ...
  void FSI<dim>::check_velocity_boundary(const unsigned int boundary_id)
  {
    ...
    if(!(param.time_integration.scheme == Parameters::TimeIntegration::Scheme::BDF2 && this->current_time_step == 1))
    {
  ...
  void FSI<dim>::run()
  {
    ...
    this->set_bdf_coefficients();

    this->current_time = param.time_integration.t_initial;
    ...
    for (unsigned int i = 0; i < param.time_integration.n_constant_timesteps; ++i, ++(this->current_time_step))
    {
      this->current_time += param.time_integration.dt;
      this->inlet_fun.set_time(current_time);
      this->fixed_mesh_position_fun.set_time(current_time);
      this->mesh_position_circle_fun.set_time(current_time);
      ...
      if (i == 0 && param.time_integration.scheme == Parameters::TimeIntegration::Scheme::BDF2)
      {
        this->set_initial_condition();
      }
      // else if (i == 1 && param.time_integration.scheme == 3)
      // {
      //   this->set_initial_condition();
      // }
      else
...
int main(int argc, char *argv[])
{
  try
  {
    ...
    ///////////////////////////////////////////////////////////////////////////////
    // This is in the process of being handled by the parameter file
    //
    BoundaryDescription boundary_description;

    boundary_description.Re = 200.;
    boundary_description.H  = 16.;
    boundary_description.D  = 1.;
    boundary_description.U  = 1.;

    //
    // Mesh position BC
    //
    boundary_description.position_fixed_boundary_names = {"Inlet", "Outlet", "NoFlux"};

    //
    // Velocity BC
    //
    boundary_description.strong_velocity_boundary_names = {"Inlet"};
    boundary_description.weak_velocity_boundary_names   = {"InnerBoundary"};
    boundary_description.noflux_velocity_boundary_names = {"NoFlux"};
    ...
    if(dim == 2)
    {
      ...
      /////////////////////////////////////////////////////////////////////////////////
      if(!param.fsi.enable_coupling)
      boundary_description.position_fixed_boundary_names.push_back("InnerBoundary");
      // boundary_description.position_moving_boundary_names = {"InnerBoundary"};
      /////////////////////////////////////////////////////////////////////////////////

      FSI<2> problem(param, boundary_description);
      problem.run();
    }
    else if(dim == 3)
    {
      ...
      FSI<3> problem(param, boundary_description);
      problem.run();
    }
    else
    {
