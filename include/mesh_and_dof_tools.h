...
/**
 * Extrait une sous-solution d'un vecteur source et l'injecte dans un vecteur de destination,
 * en gérant des espaces d'éléments finis (FESystem) potentiellement différents, 
 * tant qu'ils partagent la même Triangulation (le même maillage).
 */
template <int dim, typename VectorType>
void extract_subsolution(const DoFHandler<dim> &dof_handler_source,
                         ...
                         const std::map<unsigned int, unsigned int> &source_comp_to_dest_comp)
{
  ...
  // Pour chaque DoF local de la source, on trouve l'indice du DoF local de destination correspondant
  std::vector<unsigned int> source_to_dest_local_dof(fe_source.dofs_per_cell, numbers::invalid_unsigned_int);
  
  const std::vector<Point<dim>> &source_support_points = fe_source.get_unit_support_points();
  const std::vector<Point<dim>> &dest_support_points   = fe_dest.get_unit_support_points();
  ...
  for (unsigned int i = 0; i < fe_source.dofs_per_cell; ++i)
  {
    ...
    if (source_comp_to_dest_comp.find(comp_s) != source_comp_to_dest_comp.end())
    {
      ...
      for (unsigned int j = 0; j < fe_dest.dofs_per_cell; ++j)
      {
        ...
        // Correspondance parfaite : même composante (mappée) ET même position spatiale
  ...
  std::vector<types::global_dof_index>    local_indices_dest(fe_dest.dofs_per_cell);

  for (; cell_source != dof_handler_source.end(); ++cell_source, ++cell_dest)
  {
    // Important pour le MPI : on n'écrit que sur les cellules possédées localement
    if (cell_dest->is_locally_owned())
    {
      ...
      for (unsigned int i = 0; i < fe_source.dofs_per_cell; ++i)
      {
        if (source_to_dest_local_dof[i] != numbers::invalid_unsigned_int)
        {
          ...
          const types::global_dof_index dest_global_idx = local_indices_dest[dest_local_idx];
