...
namespace Step40
{
  ...
  LaplaceProblem<dim>::solve()
  {
    ...
    const auto &locally_owned_range = completely_distributed_solution.locally_owned_elements();

    // for (auto i : locally_owned_range)
    //   pcout << "Process " << Utilities::MPI::this_mpi_process(MPI_COMM_WORLD)
    //         << ": vector[" << i << "] = " << completely_distributed_solution[i] << std::endl;
