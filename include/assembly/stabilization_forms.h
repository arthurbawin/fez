...
namespace Assembly
{
        ...
        for (unsigned int j = 0; j < scratch.dofs_per_cell; ++j)
        {
          ...
          if (ordering.is_velocity(comp_j))
          {
            const Tensor<1, dim> dR_du_j =
              bdf_c0 * scratch.phi_u[q][j] + scratch.grad_phi_u[q][j] * u_conv +
              grad_u * scratch.phi_u[q][j] - nu * scratch.diffusion_phi_u[q][j];
          ...
          if (ordering.is_pressure(comp_j))
          {
            ...
            if (i_is_u)
            {
              const Tensor<1, dim> supg_test =
                scratch.grad_phi_u[q][i] * u_conv;
              local_matrix_ij += tau * supg_test * scratch.grad_phi_p[q][j];
            }
          }
  ...
  inline void assemble_ns_rhs_stabilization(const ComponentOrdering &ordering,
                                            const ScratchData       &scratch,
                                            const bool  stabilization_enabled,
    ...
    for (unsigned int q = 0; q < scratch.n_q_points; ++q)
    {
      ...
      for (unsigned int i = 0; i < scratch.dofs_per_cell; ++i)
      {
        ...
        if (ordering.is_velocity(comp_i))
          local_rhs_i -= tau * ((scratch.grad_phi_u[q][i] * u_conv) *
  ...
  template <int dim, typename ScratchData, typename VectorType>
  inline void assemble_chns_rhs_stabilization(const ComponentOrdering &ordering,
                                              const ScratchData       &scratch,
                                              const bool  stabilization_enabled,
                                              VectorType &local_rhs)
  {
    ...
    for (unsigned int q = 0; q < scratch.n_q_points; ++q)
    {
      ...
      for (unsigned int i = 0; i < scratch.dofs_per_cell; ++i)
      {
        ...
        if (tau_mom > 0.)
        {
          ...
          if (ordering.is_velocity(comp_i))
            local_rhs_i -= tau_mom * ((scratch.grad_phi_u[q][i] * u_conv) *
                                      scratch.strong_residual_momentum[q]);
        }
                                     ...
                                     MatrixType &local_matrix)
  {
    ...
    for (unsigned int q = 0; q < scratch.n_q_points; ++q)
    {
      ...
      for (unsigned int i = 0; i < scratch.dofs_per_cell; ++i)
      {
        ...
        for (unsigned int j = 0; j < scratch.dofs_per_cell; ++j)
        {
          if (coupling_table[comp_i][scratch.components[j]] != DoFTools::always)
            continue;
          ...
          if (tau_mom > 0. && (i_is_u || i_is_p))
          {
            if (ordering.is_velocity(comp_j))
            {
              ...
              const Tensor<1, dim> dR_du_j =
                ...
                scratch.stabilization_nu_eff[q] * scratch.diffusion_phi_u[q][j];
              ...
              if (i_is_u)
              {
                local_matrix_ij += tau_mom * (grad_phi_u_i * phi_u_j) *
                                   scratch.strong_residual_momentum[q];
                local_matrix_ij += tau_mom * supg_test_momentum * dR_du_j;
              }

              const Tensor<1, dim> extra_dR =
                scratch.stabilization_inv_rho[q] * diffusive_flux_factor *
              ...
              if (i_is_u)
                local_matrix_ij += tau_mom * supg_test_momentum * extra_dR;
            }

            if (ordering.is_pressure(comp_j))
            {
              const Tensor<1, dim> scaled_grad_p_j =
                scratch.stabilization_inv_rho[q] * scratch.grad_phi_p[q][j];

              if (i_is_p)
                local_matrix_ij -= tau_mom * scaled_grad_p_j * grad_phi_p_i;
              if (i_is_u)
                local_matrix_ij +=
            ...
            if (ordering.is_tracer(comp_j))
            {
              ...
              const Tensor<1, dim> dR_dphi_j =
                scratch.shape_phi[q][j] *
                  (dinvrho_dphi * (diffusive_flux_factor *
                ...
                2. * scratch.stabilization_inv_rho[q] *
                  scratch.derivative_dynamic_viscosity_wrt_tracer[q] * eps_u *
                  scratch.grad_shape_phi[q][j];

              if (i_is_p)
                local_matrix_ij -= tau_mom * dR_dphi_j * grad_phi_p_i;
              if (i_is_u)
                local_matrix_ij += tau_mom * supg_test_momentum * dR_dphi_j;
            }
            ...
            if (ordering.is_potential(comp_j))
            {
              ...
              if (i_is_u)
                local_matrix_ij += tau_mom * supg_test_momentum * dR_dmu_j;
            }
            ...
            if constexpr (with_moving_mesh)
              if (ordering.is_position(comp_j))
              {
                ...
                const Tensor<1, dim> dR_dx_j =
                  ...
                  scratch.stabilization_inv_rho[q] * scratch.tracer_values[q] *
                    dgrad_mu_dx_j;

                if (i_is_p)
                {
                  const Tensor<1, dim> dgrad_phi_p_i_dx_j = -GT * grad_phi_p_i;
                  local_matrix_ij -= tau_mom * dR_dx_j * grad_phi_p_i;
                  local_matrix_ij -= tau_mom *
                                     scratch.strong_residual_momentum[q] *
                                     dgrad_phi_p_i_dx_j;
                  local_matrix_ij -= tau_mom *
                                     scratch.strong_residual_momentum[q] *
                ...
                if (i_is_u)
                {
                  const Tensor<2, dim> dgrad_phi_u_i_dx_j = -grad_phi_u_i * G;
          ...
          if (tau_tracer > 0. && i_is_tracer)
          {
            ...
            if (ordering.is_tracer(comp_j))
            {
              const double dR_dphi_j =
                ...
                ddM_dphi2 * scratch.shape_phi[q][j] *
                  (scratch.tracer_gradients[q] *
                   scratch.potential_gradients[q]) -
                dM_dphi * (scratch.grad_shape_phi[q][j] *
                           scratch.potential_gradients[q]);
              local_matrix_ij += tau_tracer * supg_test_tracer * dR_dphi_j;
            }
            ...
            if (ordering.is_potential(comp_j))
            {
              ...
              local_matrix_ij += tau_tracer * supg_test_tracer * dR_dmu_j;
            }
            if constexpr (with_moving_mesh)
              if (ordering.is_position(comp_j))
              {
                ...
                local_matrix_ij += tau_tracer * supg_test_tracer * dR_dx_j;
                local_matrix_ij += tau_tracer * d_supg_test_dx_j *
                                   scratch.strong_residual_tracer[q];
                local_matrix_ij += tau_tracer * supg_test_tracer *
                                   scratch.strong_residual_tracer[q] * trG;
              }
          }
