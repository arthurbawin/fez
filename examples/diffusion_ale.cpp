  ...
  public:
                              ...
                              Vector<double>   &values) const override
    {
      ...
      if constexpr (dim == 2)
      {
        ...
        // For a translation in the x-direction
        // values[1] = 0.;
        // values[2] = 0.;
#else
...
#if defined(WITH_ALE)
#  if defined(QUADRATIC_DISPLACEMENT)
        gradients[u_lower + 0][0] =
          -(Gdot / (2.0 * k)) - (phi * G * (2.0 * x - 1.0)) / (8.0 * k);
        ...
        // gradients[u_lower + 0][1] = -(Gdot + G * phidot / 4. * (2. * y
        // - 1.));
#  else
        DEAL_II_NOT_IMPLEMENTED();
...
# endif
  ...
  public:
    ...
    LA::MPI::Vector initial_position_dofs;
    LA::MPI::Vector current_position_dofs;
  ...
  void MMS<dim>::setup_system()
  {
    ...
    initial_position_dofs.reinit(locally_owned_dofs, mpi_communicator);
    current_position_dofs.reinit(locally_owned_dofs, mpi_communicator);

    exact_solution.reinit(locally_owned_dofs, mpi_communicator);
    exact_solution_with_ghosts.reinit(locally_owned_dofs,
                                      locally_relevant_dofs,
                                      mpi_communicator);
    ...
    // Also set the initial position in the dedicated vector
    // VectorTools::get_position_vector(*fixed_mapping,
    //                                  dof_handler,
    //                                  initial_position_dofs,
    //                                  fe.component_mask(mesh_position));
    // initial_position_dofs.compress(VectorOperation::insert);

    // current_position_dofs = initial_position_dofs;
    // current_position_dofs.compress(VectorOperation::insert);

    // Save the position dof indices
    // pos_dof_indices = DoFTools::extract_dofs(dof_handler,
    //                                          fe.component_mask(mesh_position));
    ...
    mapping = std::make_unique<MappingFEField<dim, dim, LA::MPI::Vector>>(
      dof_handler, local_evaluation_point, fe.component_mask(mesh_position));
#endif
  }
