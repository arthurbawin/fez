...
{
  inline void simple_mesh_forcing_factor_and_jacobian(const double phase_value,
                                                      double      &factor,
  ...
  inline void regularized_band_mesh_forcing_factor_and_jacobian(
    ...
    double      &factor_jacobian)
  {
    ...
    const double denominator       = 1. - mff_band_factor * mff_band_factor *
                                      regularized_phase * regularized_phase;

    factor          = regularized_phase / denominator;
    factor_jacobian = regularized_jac *
                      (1. + mff_band_factor * mff_band_factor *
                              regularized_phase * regularized_phase) /
    ...
    double                              &factor_jacobian)
  {
    if (cahn_hilliard.mesh_forcing_law ==
      ...
      simple_mesh_forcing_factor_and_jacobian(phase_value,
                                              factor,
                                              factor_jacobian);
    else
  ...
  {
    const double enlarged_epsilon = with_enlarged ?
                                      cahn_hilliard.epsilon_interface_enlarged :
      ...
      for (unsigned int i = 0; i < scratch.dofs_per_cell; ++i)
      {
        ...
        for (unsigned int j = 0; j < scratch.dofs_per_cell; ++j)
        {
          ...
          if (ordering.is_velocity(comp_j))
            local_ij -=
              phi_x_i * (cahn_hilliard.mff_transport_factor *
                         (enlarged_epsilon * enlarged_epsilon) *
          ...
          if (ordering.is_position(comp_j))
          {
            ...
            local_ij -=
              ...
              (cahn_hilliard.mff_transport_factor *
               (enlarged_epsilon * enlarged_epsilon) *
               ((-bdf_c0) * scratch.phi_x[q][j] * enlarged_phase_gradient *
                  enlarged_phase_gradient +
                (u_conv * transported_enlarged_gradient) *
                  enlarged_phase_gradient +
                enlarged_u_dot_grad_phase * transported_enlarged_gradient));

            if constexpr (with_enlarged)
              local_ij -=
                phi_x_i * (cahn_hilliard.mff_enlarged_compression_factor *
                           enlarged_epsilon * enlarged_factor *
                           transported_enlarged_gradient);

            local_ij -=
              phi_x_i * (cahn_hilliard.mff_physics_compression_factor *
                         cahn_hilliard.epsilon_interface * tracer_factor *
                ...
                (cahn_hilliard.mff_enlarged_compression_factor *
                 ...
                 cahn_hilliard.mff_transport_factor *
                   ...
                   ((u_conv * scratch.grad_shape_psi[q][j]) *
                      enlarged_phase_gradient +
                    enlarged_u_dot_grad_phase * scratch.grad_shape_psi[q][j]));
