    ...
    for (unsigned int cycle = 0; cycle < 1; ++cycle)
      {
        ...
        if (cycle == 0)
          {
            // Triangulation<dim> triquad;
            // GridGenerator::hyper_cube(triquad, -1, 1);
            // triquad.refine_global(4);
            // GridGenerator::convert_hypercube_to_simplex_mesh (triquad,
            //                                       triangulation);
            GridIn<dim> grid_in;
            grid_in.attach_triangulation(triangulation);
            std::ifstream input_file("../data/meshes/square_minus1_to_1.msh");
            AssertThrow(input_file, ExcMessage("Could not open mesh.msh"));
