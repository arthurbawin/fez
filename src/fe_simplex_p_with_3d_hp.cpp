...
template <int dim, int spacedim>
std::string FE_SimplexP_3D_hp<dim, spacedim>::get_name() const
{
  ...
  const FiniteElement<dim, spacedim> &fe_other) const
{
  ...
  if (dynamic_cast<const FE_SimplexP_3D_hp<dim, spacedim> *>(&fe_other) !=
      nullptr)
  {
  ...
  const unsigned int) const
{
  ...
  //     // to have two indices i1, i2 and j1, j2 to characterize the dofs on
  //     the
  ...
  //                                           [0]) < 1e-14))
  //               identities.emplace_back(i1 * (p - 1) + i2, j1 * (q - 1) +
  //               j2);
  ...
  //     return std::vector<std::pair<unsigned int, unsigned int>>();
  //   }
  // else if (fe_other.n_unique_faces() == 1 && fe_other.n_dofs_per_face(0) ==
  // 0)
  //   {
  //     // if the other element has no elements on faces at all,
  //     // then it would be impossible to enforce any kind of
