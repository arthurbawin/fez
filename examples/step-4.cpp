...
void Step4<dim>::solve()
{
  const unsigned int n_rows = system_matrix.m();
  const unsigned int n_cols = system_matrix.n();
  const unsigned int n = system_rhs.size();

  std::vector<unsigned int> local_rows_per_process(1, dof_handler.n_dofs());
  std::vector<unsigned int> local_cols_per_process(1, dof_handler.n_dofs());
  unsigned int this_process = 0;

  LinearAlgebraPETSc::MPI::SparseMatrix petsc_matrix(Utilities::MPI::MPI_COMM_SELF, sparsity_pattern, local_rows_per_process, local_cols_per_process, this_process);
  LinearAlgebraPETSc::MPI::Vector petsc_rhs(Utilities::MPI::MPI_COMM_SELF, n, n);
  LinearAlgebraPETSc::MPI::Vector petsc_solution(Utilities::MPI::MPI_COMM_SELF, n, n);

  // 1. Convert system_matrix to PETSc MPI SparseMatrix
  for (unsigned int i = 0; i < n_rows; ++i)
  {
    for (SparseMatrix<double>::const_iterator it = system_matrix.begin(i);
         it != system_matrix.end(i); ++it)
    {
  ...
  // 2. Convert RHS and solution to PETSc MPI Vectors
  for (unsigned int i = 0; i < n_rows; ++i)
    petsc_rhs[i] = system_rhs[i];

  petsc_rhs.compress(VectorOperation::insert);

  // 3. Solve using PETSc MUMPS
  SolverControl solver_control(100, 1e-10, true, true);
  PETScWrappers::SparseDirectMUMPS solver(solver_control);
  solver.solve(petsc_matrix, petsc_solution, petsc_rhs);

  // Using petsc's LU
  // SolverControl solver_control;
  // PETScWrappers::SolverPreOnly preonly(solver_control);
  // PETScWrappers::PreconditionLU preconditioner(petsc_matrix);
  // preonly.solve(petsc_matrix, petsc_solution, petsc_rhs, preconditioner);

  // 4. Copy back to solution vector
  for (unsigned int i = 0; i < n_rows; ++i)
    solution[i] = petsc_solution[i];

  std::cout << "   Solved using PETSc MUMPS direct solver." << std::endl;
}
