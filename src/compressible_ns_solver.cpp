  ...
  Vector<double>   &values) const
{
  ...
  // Incompressible equations at fixed density
  {
    ...
    // for div(u) + alpha_r/(alpha_r p^* + 1)[dp^*dt + u dot gradp^*] - beta_r/(beta_r T^* + 1)[dT^*dt + u dot gradT^*] - f = 0 
    // -> f = div(u_mms) + alpha_r/(alpha_r p^*_mms + 1)[dp^*_mmsdt + u_mms dot gradp^_mms*] - beta_r/(beta_r T^*_mms + 1)[dT^_mms*dt + u_mms dot gradT^*_mms]
    values[p_lower] = -(div_u + a_p * (dpdt_ex + uDotGrad_p)- b_T * (dTdt_ex + uDotGrad_T));
  ...
  for (const auto &[id, bc] : this->param.heat_bc)
  {
    ...
    else if (bc.type == BoundaryConditions::Type::input_function)
    {
      ...
      VectorTools::interpolate_boundary_values(*mapping,
                                               this->dof_handler,
                                               id,
                                               ComponentwiseFlowPressure<dim>(this->ordering->t_lower,this->ordering->n_components,*bc.temperature),
                                               this->nonzero_constraints,
                                               temperature_mask);
    }
  }
...
void CompressibleNSSolver<dim>::assemble_local_matrix(
  ...
  CopyData                                             &copy_data)
{
  // AssertThrow(false, ExcMessage("Not using analytical jacobian matrix"));
    ...
    for (unsigned int i = 0; i < scratchData.dofs_per_cell; ++i)
    {
      ...
      for (unsigned int j = 0; j < scratchData.dofs_per_cell; ++j)
      {
        ...
        if (i_is_u && j_is_u)
        {
          ...
          local_matrix_ij += phi_u[i] * rho * (grad_phi_u[j] * present_velocity_values + present_velocity_gradients * phi_u[j]);
        ...
        if (i_is_p && j_is_u)
        {
          ...
          local_matrix_ij += phi_p[i] * a_p * phi_u[j] * present_pressure_gradients;
          local_matrix_ij += - phi_p[i] * b_T * phi_u[j] * present_temperature_gradients;
        }
        ...
        if (i_is_p && j_is_p)
        {
          ...
          local_matrix_ij += phi_p[i] * a_p * present_velocity_values * grad_phi_p[j];
        }
        ...
        if (i_is_p && j_is_T)
        {
          ...
          local_matrix_ij += - phi_p[i] * b_T * present_velocity_values * grad_phi_T[j];
        }
        ...
        if (i_is_T && j_is_u)
        {
          ...
          local_matrix_ij += phi_T[i] * rho * cp * phi_u[j] * present_temperature_gradients;
        ...
        if (i_is_T && j_is_T)
        {
          ...
          local_matrix_ij += phi_T[i] * rho * cp  * present_velocity_values * grad_phi_T[j];
      ...
      if (face->at_boundary())
      {
        ...
        if (bc_fluid.type == BoundaryConditions::Type::input_function && bc_fluid.impose_pressure)
        {
          for (unsigned int q = 0; q < scratchData.n_faces_q_points; ++q)
          {
            ...
            const auto &grad_phi_u_face = scratchData.grad_phi_u_face[i_face][q];
            const auto &sym_grad_phi_u_face = scratchData.sym_grad_phi_u_face[i_face][q];
            ...
            for (unsigned int i = 0; i < scratchData.dofs_per_cell; ++i)
            {
              ...
              for (unsigned int j = 0; j < scratchData.dofs_per_cell; ++j)
              {
                ...
                if (i_is_u && j_is_u)
                {
                  ...
                  local_matrix_ij += (-mu * 2.0* sym_grad_phi_u_face[j] * n * phi_u_face[i] + 2.0/3.0 * mu * div_phi_u_face[j] * n * phi_u_face[i]);
                }
  ...
  CopyData                                             &copy_data)
{
  ...
  for (unsigned int q = 0; q < scratchData.n_q_points; ++q)
  {
    ...
    const double rho = rho_ref * ((alpha_r * present_pressure_values + 1.0) / (beta_r * present_temperature_values + 1.0));
    ...
    for (unsigned int i = 0; i < scratchData.dofs_per_cell; ++i)
    {
      double local_rhs_i = -(
        ...
        + a_p * (dpdt + present_velocity_values * present_pressure_gradients)  * phi_p[i]
        - b_T * (dTdt + present_velocity_values * present_temperature_gradients) * phi_p[i]
        + source_term_pressure * phi_p[i]

        //Momentum
        + rho * (dudt * phi_u[i] + present_velocity_gradients * present_velocity_values * phi_u[i])
        ...
        // Energy
        + rho * cp * (dTdt + present_velocity_values * present_temperature_gradients) * phi_T[i]
        - (dpdt + present_velocity_values * present_pressure_gradients) * phi_T[i]
      ...
      //   // const Tensor<1, dim> u_diff = present_velocity_values - u_exact;
      //   // local_rhs_i = -(u_diff * phi_u[i]);
      // }
      ...
      //   // local_rhs_i = -(present_temperature_values * phi_T[i]);
      // }
      ...
      if (face->at_boundary())
      {
              ...
              .type == BoundaryConditions::Type::pressure_mms)
        {
          ...
          for (unsigned int q = 0; q < scratchData.n_faces_q_points; ++q)
          {
            ...
            const auto div_u_exact = scratchData.exact_face_velocity_divergences[i_face][q];
            const double p_exact = scratchData.exact_face_pressure_values[i_face][q];
            // const auto &grad_T_exact = scratchData.exact_face_temperature_gradients[i_face][q];

            const SymmetricTensor<2, dim> D = symmetrize(grad_u_exact);
            const SymmetricTensor<2, dim> tau = 2.0 * mu * D - 2.0/3.0 * mu * div_u_exact * identity_tensor;
            ...
            for (unsigned int i = 0; i < scratchData.dofs_per_cell; ++i)
            {
              local_rhs(i) -= ((p_exact * identity_tensor - tau) * n * phi_u_face[i]) * face_JxW;

              // local_rhs(i) -= (k * (grad_T_ex * n) * phi_T_face[i]) * face_JxW;          
            }
          }
        }
        ...
        if (bc_fluid.type == BoundaryConditions::Type::input_function && bc_fluid.impose_pressure)
        {
          ...
          for (unsigned int q = 0; q < scratchData.n_faces_q_points; ++q)
          {
            ...
            const auto &present_face_velocity_gradients = scratchData.present_face_velocity_gradients[i_face][q];
            const auto &present_face_velocity_divergence = scratchData.present_face_velocity_divergence[i_face][q];
            const double &pressure_bc = scratchData.face_input_pressure_values[i_face][q];

            const SymmetricTensor<2, dim> identity_tensor = unit_symmetric_tensor<dim>();
            const SymmetricTensor<2, dim> D = symmetrize(present_face_velocity_gradients);
            const SymmetricTensor<2, dim> tau = 2.0 * mu * D - 2.0/3.0 * mu * present_face_velocity_divergence* identity_tensor;

            const auto &phi_u_face = scratchData.phi_u_face[i_face][q];

            for (unsigned int i = 0; i < scratchData.dofs_per_cell; ++i)
            {
              local_rhs(i) -= ((pressure_bc * identity_tensor - tau) * n * phi_u_face[i]) * face_JxW;
            }
          }
        }
