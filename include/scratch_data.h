...
class ScratchData
{
              ...
              const std::vector<VectorType2>      &previous_solutions)
  {
    ...
    //
    if constexpr (std::is_same<VectorType1, dealii::LinearAlgebraPETSc::MPI::Vector>::value)
    {
      //
    ...
    {
      static_assert(false,
                    "reinit expects dealii::LinearAlgebraPETSc::MPI::Vector or std::vector<double>");
    }
    ...
    for (const auto i_face : cell->face_indices())
    {
      ...
      {
        static_assert(
          false, "reinit expects dealii::LinearAlgebraPETSc::MPI::Vector or std::vector<double>");
      }
