...
if test ! -d "$SRC/src" -o ! -d "$SRC/include" -o ! -f "$SRC/CMakeLists.txt" ; then
    echo "Usage:"
...
ARGS=("-D" "CMAKE_EXPORT_COMPILE_COMMANDS=ON" "-D" "CMAKE_BUILD_TYPE=Debug" "-D" "REFERENCE_CELL_WITH_TEMPLATE_PARAMETER=1" "$@")
...
run-clang-tidy -p . -quiet -header-filter "$SRC/include/*" --extra-arg=-std=c++17 --extra-arg='-DCLANG_TIDY' 2>error.txt >output.txt
