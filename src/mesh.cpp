...
namespace MeshTools
{
                                ...
                                std::map<std::string, unsigned int> &name2tag)
  {
    ...
    while (std::getline(in, line))
    {
      ...
      if (line == "$PhysicalNames")
      {
        ...
        unsigned int num;
        AssertThrow(in >> num,
                    ExcMessage("Invalid $PhysicalNames section in " +
                               meshFile));
                        ...
                        const bool          convert_to_tets = false)
  {
    ...
    AssertThrow(
      ...
      ExcMessage(
        ...
        ": "
        "[remaining parameters]), but the given parameters are not separated "
        "by "
    ...
    GridGenerator::generate_from_name_and_arguments(
      tria, "subdivided_hyper_rectangle", updated_param);
                          ...
                          const ParameterReader<dim> &param)
  {
     ...
     * not part of any physical entity, which are read by deal.II with boundary
     * id 0. Filtering them out of the mesh can be very tedious, here they are
     * simply set as unused, but associated boundary conditions are still
     * required in the parameter file. This should be treated here.
     */
    ...
    for (const auto &[id, count] : boundary_count)
      AssertThrow(
        ...
        ExcMessage(
          ...
          "geometric entity (i.e., a boundary curve or surface) in the mesh "
          "that "
    ...
    const ParameterReader<dim>                  &param)
  {
    ...
    AssertThrow(
      param.mesh.name2id.count(bc.gmsh_name) == 1,
  ...
  template <int dim>
  void
  check_boundary_conditions_compatibility(const ParameterReader<dim> &param)
  {
