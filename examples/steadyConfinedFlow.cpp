...
namespace Step55
{
  ...
  StokesProblem<dim>::solve()
  {
    ...
    std::cout << "Rank " << Utilities::MPI::this_mpi_process(mpi_communicator)
          << ": Nonzeros in matrix = " << system_matrix.n_nonzero_elements() << std::endl;
    ...
    PETScWrappers::PreconditionLU pLU(system_matrix);
    preonly.solve(system_matrix, distributed_solution, system_rhs, pLU);
