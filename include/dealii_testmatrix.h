...
template <typename SP>
inline void FDMatrix::five_point_structure(SP &structure) const
{
...
template <typename SP>
inline void FDMatrix::nine_point_structure(SP &structure) const
{
  for (unsigned int i = 0; i <= ny - 2; ++i)
  {
    for (unsigned int j = 0; j <= nx - 2; ++j)
...
template <typename MatrixType>
void FDMatrix::nine_point(MatrixType &A, bool) const
{
  for (unsigned int i = 0; i <= ny - 2; ++i)
  {
    for (unsigned int j = 0; j <= nx - 2; ++j)
...
template <typename MatrixType>
inline void FDMatrix::five_point(MatrixType &A, bool nonsymmetric) const
{
  for (unsigned int i = 0; i <= ny - 2; ++i)
  {
    for (unsigned int j = 0; j <= nx - 2; ++j)
...
template <typename MatrixType>
inline void FDMatrix::upwind(MatrixType &A, bool back) const
{
  for (unsigned int i = 0; i <= ny - 2; ++i)
  {
    for (unsigned int j = 0; j <= nx - 2; ++j)
...
template <typename number>
inline void FDMatrix::gnuplot_print(std::ostream         &s,
                                    const Vector<number> &V) const
{
  for (unsigned int i = 0; i <= ny - 2; ++i)
  {
    for (unsigned int j = 0; j <= nx - 2; ++j)
    {
      // Number of the row to be entered
      unsigned int row = j + (nx - 1) * i;
      s << (j + 1) / (float)nx << '\t' << (i + 1) / (float)ny << '\t' << V(row)
        << std::endl;
    }
    s << std::endl;
  }
