...
template <int dim,
          ...
          typename CopyData,
          typename VectorType>
void Verification::compute_local_matrix_finite_differences(
  const typename DoFHandler<dim>::active_cell_iterator &cell,
  MainClass                                            &main_object,
  void (MainClass::*assemble_local_rhs)(const Iterator &,
                                        ScratchData &,
...
template <int dim,
          ...
          typename CopyData,
          typename VectorType>
std::pair<double, double> Verification::compare_analytical_matrix_with_fd(
  const DoFHandler<dim> &dof_handler,
  const unsigned int     n_dofs_per_cell,
  MainClass             &main_object,
  void (MainClass::*assemble_local_matrix)(const Iterator &,
  ...
  const double       relative_tolerance)
{
  ...
  for (const auto &cell : dof_handler.active_cell_iterators())
  {
    ...
    if (max_abs_err > absolute_tolerance && max_rel_err > relative_tolerance)
    {
      ...
      {
        if constexpr (dim == 2)
          outfile << cell->vertex(iv)[0] << "," << cell->vertex(iv)[1] << ",0."
  ...
  max_absolute_error_over_all_elements =
    Utilities::MPI::max(max_absolute_error_over_all_elements, mpi_communicator);
  max_relative_error_over_all_elements =
    Utilities::MPI::max(max_relative_error_over_all_elements, mpi_communicator);
