...
public:
                       ...
                       const unsigned int component = 0) const override
  {
    ...
    const double sz = sin(M_PI * z);
    const double cz = cos(M_PI * z);

    if (component == 0) // f
      return sx * cy * sz;
    else if (component == 1) // df/dx
      return M_PI * cx * cy * sz;
    else if (component == 2) // df/dy
      return -M_PI * sx * sy * sz;
    else if (component == 3) // df/dz
      return M_PI * sx * cy * cz;
    else if (component == 4) // d2f/dx2
      return -M_PI * M_PI * sx * cy * sz;
    else if (component == 5) // d2f/dxdy
      return -M_PI * M_PI * cx * sy * sz;
    else if (component == 6) // d2f/dxdz
      return M_PI * M_PI * cx * cy * cz;
    else if (component == 7) // d2f/dydx
      return -M_PI * M_PI * cx * sy * sz;
    else if (component == 8) // d2f/dy2
      return -M_PI * M_PI * sx * cy * sz;
    else if (component == 9) // d2f/dydz
      return -M_PI * M_PI * sx * sy * cz;
    else if (component == 10) // d2f/dzdx
      return M_PI * M_PI * cx * cy * cz;
    else if (component == 11) // d2f/dzdy
      return -M_PI * M_PI * sx * sy * cz;
    else if (component == 12) // d2f/dz2
      return -M_PI * M_PI * sx * cy * sz;
...
void test_ppr(const unsigned int field_polynomial_degree)
{
  ...
  for (unsigned int i_conv = 0; i_conv < 2; ++i_conv)
  {
